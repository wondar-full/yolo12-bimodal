# 🚀 SOLR快速启动指南 - yolo12-bimodal

> **当前项目**: yolo12-bimodal  
> **新增功能**: SOLR (Small Object Loss Reweighting)  
> **目标**: 提升中等目标检测性能,缩小与RemDet的差距
...
| 文件 | 大小 | 用途 |
|------|------|------|
| `ultralytics/utils/solr_loss.py` | ~600行 | SOLR核心实现 |
| `train_depth_solr.py` | ~500行 | SOLR训练脚本 |
| `代码审查报告_SOLR集成.md` | ~400行 | 代码审查与问题分析 |
...
**预期提升**:
- AP@0.50: +2.5~3.5% (从32.57% → 35~36%)
- AP_m: +2~3% (从28.86% → 31~32%)
- AP_s: +1~1.5% (从9.61% → 10.5~11%)
...
**可能原因**:
1. SOLR权重不够大 (尝试medium_weight=2.5)
2. 训练不够充分 (等待更多epochs)
3. 数据问题 (检查深度图归一化)
...
**关键**: SOLR主要提升**中等目标**(AP_m),这是RemDet的主要优势区域!
...
**下一步**:
1. 本地测试SOLR模块 (`python -m ultralytics.utils.solr_loss`)
2. 上传到服务器
3. 运行10 epochs快速测试
4. 启动300 epochs完整训练
